...
namespace VoidMail
{
    ...
    public class NSTextBox : Control
    {
        ...
        {
            if (Base != null && Base.IsHandleCreated)
            {
                SendMessage(Base.Handle, WM_VSCROLL, (IntPtr)SB_BOTTOM, IntPtr.Zero);
            }
        }
    ...
    public class NSComboBox : ComboBox
    {
        ...
        public void ClearComboBoxItems()
        {
            if (this?.IsHandleCreated == true)
            {
                ...
                {
                    this.Items.Clear();
                    this.Height = this.Font.Height + 6; // Adjust the height based on your preference
                    this.Invalidate();
                    this.Refresh();
                }));
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (this.Items.Count > 0 && !string.IsNullOrEmpty(Text))
            {
                SZ1 = ThemeModule.G.MeasureString(Text, Font);
        ...
        protected override void OnDrawItem(DrawItemEventArgs e)
        {
            ...
            if (e.Index >= 0 && e.Index < Items.Count)  // Check if the index is valid
            {
                e.Graphics.DrawString(GetItemText(Items[e.Index]), Font, Brushes.White, e.Bounds);
            }
        }
